...
def get_base_config():
    ...
    """just load the config which the dagobahd.yml is exists add by tommydong"""

    file_open = open('/root/dagobahd.yml')
...
def configure_app():
    print('start app now')
    app.debug = get_conf(config, 'Dagobahd.debug', True)
    ...
    app.config['APP_PORT'] = get_conf(config, 'Dagobahd.port', '9966')
...
def get_backend(config):
    ...
    if backend_string is None:
        ...
        try:
            ...
            print('import MongoBackend failed')
